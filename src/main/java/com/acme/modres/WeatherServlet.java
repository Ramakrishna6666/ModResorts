...
public class WeatherServlet extends HttpServlet {
  ...
  // blocker-7: Secret name stored in environment variable; actual key retrieved from AWS Secrets Manager
  private static final String WEATHER_API_SECRET_NAME = System.getenv("WEATHER_API_SECRET_NAME") != null
      ? System.getenv("WEATHER_API_SECRET_NAME") : "modresorts/weather-api-key";

  // Fallback: local OS environment variable key name for backward compatibility
  ...
  /**
   * blocker-7: Retrieve the weather API key from AWS Secrets Manager.
   * Falls back to environment variable for local development compatibility.
   */
  private String getWeatherApiKey() {
    ...
    try {
      ...
      if (secretValue != null && !secretValue.trim().isEmpty()) {
        logger.info("Weather API key retrieved from AWS Secrets Manager.");
        return secretValue.trim();
      }
    } catch (SecretsManagerException e) {
      logger.warning("Could not retrieve secret from AWS Secrets Manager: " + e.getMessage()
          + ". Falling back to environment variable.");
    } catch (Exception e) {
      logger.warning("Unexpected error retrieving secret: " + e.getMessage()
          + ". Falling back to environment variable.");
    }
  ...
  protected void doGet(HttpServletRequest request,
    ...
    // blocker-7: Retrieve API key from AWS Secrets Manager instead of hardcoded source
