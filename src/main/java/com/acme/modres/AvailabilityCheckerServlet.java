...
public class AvailabilityCheckerServlet extends HttpServlet {
  ...
  protected int exportRevervations(String selectedDateStr) {
    // Use try-with-resources to ensure S3Client is properly closed
    try (S3Client s3Client = S3Client.builder()
        .region(Region.of(AWS_REGION))
        .build()) {
      
      // Read reservations.json from classpath
      InputStream reservationsStream = getClass().getClassLoader().getResourceAsStream("reservations.json");
      ...
      try (InputStream stream = reservationsStream) {
        // Upload to S3
        ...
        logger.severe("Failed to read or upload reservations: " + e.getMessage());
        e.printStackTrace();
        return -1;
      }
